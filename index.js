...
bot.on('ready', () => {
    ...
    bot.user.setActivity('Happiness!', {type: 'PLAYING'}).catch(console.error);
})

bot.on('guildMemberAdd', member => {
    bot.commands.get('greetings').execute(member, RichEmbed, colour);
});
...
bot.on('message', message => {
    ...
    {
        bot.commands.get('Serene').execute(message);
    ...
    if(message.content.substring(0,Prefix.length) === Prefix)
    {
        ...
        {
            case'':
                message.reply("You called me? My prefix is Serene!");
                break;
